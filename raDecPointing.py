...
az_rad = math.asin(math.cos(h_rad)*math.cos(dec_rad)*math.cos(lat_rad) + math.sin(dec_rad)*math.sin(lat_rad))
az_deg = math.degrees(az_rad)

# solve for elevation in radians
el_rad = math.atan( (-1*math.sin(h_rad)) / (math.tan(dec_rad)*math.cos(lat_rad) - math.sin(lat_rad)*math.cos(h_rad)) )
# el_rad = math.asin((-1*math.cos(dec_rad)*math.sin(h_rad))/(math.cos(az_rad))) # old version
